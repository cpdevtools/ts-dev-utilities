...
| ------------------ | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `status [--check]` | One row per package per install location, derived from the real symlink targets.            | Only with `--check`: any entry is `published` or `not-symlink`, or any link was reset by an install. |
| `link`             | Repoint installed entries at the checkouts, recording originals in the sidecar.             | Any result is `refused` (unbuilt checkout). Running under CI is an error.                            |
